...
export function getSignals({ rsi, ma20, ma50, volRatio, intraDayPct, prevMA20, prevMA50, macd, bb, vwap, donchian, weeklyRsi }) {
  ...
  if (volRatio !== null && volRatio > 1.5) {
    const volLabel = volRatio >= 3 ? 'MUY ALTO' : volRatio >= 2 ? 'ALTO' : 'ELEVADO';
    if (intraDayPct !== null && intraDayPct > 0)
      signals.push({ type: 'HIGH_VOLUME_BULL', dir: 'compra', detail: `Volumen ${volLabel} ${volRatio.toFixed(1)}x promedio con precio subiendo — demanda institucional` });
    else if (intraDayPct !== null && intraDayPct < 0)
      signals.push({ type: 'HIGH_VOLUME_BEAR', dir: 'venta', detail: `Volumen ${volLabel} ${volRatio.toFixed(1)}x promedio con precio cayendo — presión vendedora` });
    else
      signals.push({ type: 'HIGH_VOLUME', dir: 'neutral', detail: `Volumen ${volLabel} ${volRatio.toFixed(1)}x promedio` });
  }
