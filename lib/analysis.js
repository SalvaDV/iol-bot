...
import { calcRSI, calcMA, calcMACD, calcBollinger, getSignals, decideDirection } from './indicators.js';
...
async function fetchTicker(token, sym) {
  try {
    ...
    const signals = getSignals({ rsi, ma20, ma50, volRatio, intraDayPct, prevMA20, prevMA50, macd, bb });
    const dir = decideDirection(signals);

    return { sym, ultimo, rsi, ma20, ma50, macd, bb, intraDayPct, volRatio, signals, dir, _mercado: mercado };
  } catch {
    return null;
  }
}
