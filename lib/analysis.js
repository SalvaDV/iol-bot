import { getToken, getPortfolio, getCuenta, getCotizacion, getHistorial, extractPrecio } from './iol.js';
import { calcRSI, calcMA, getSignals, decideDirection } from './indicators.js';
import { sendMessage } from './telegram.js';

// Instrumentos BCBA que IOL puede cotizar via /api/v2/cotizaciones/titulos/bcba/{ticker}
// CEDEARs (NVDA, AAPL, etc.) excluidos — la API de IOL devuelve 400 para ese endpoint
...
export async function runAnalysis() {
  ...
  let efectivo = 0;
  try {
    efectivo = cuenta.cuentas?.[0]?.disponible ?? 0;
  } catch { /* keep 0 */ }
...
// Retorna todos los resultados técnicos sin enviar mensaje — usado por api/analyze.js
export async function fetchAllTechnicals(token) {
  const results = await withTimeout(
    Promise.all(WATCHLIST.map(sym => fetchTicker(token, sym))),
    ANALYSIS_TIMEOUT_MS
  );
