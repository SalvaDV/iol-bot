...
ESTRUCTURA:
• propuestas_iol: hasta 4 propuestas distintas. Incluí acá también CEDEARs crypto (COIN, MSTR, MARA, RIOT, HOOD). [] si no hay buenas opciones.
...
CRITERIO IOL CRYPTO: usá CEDEARs listados en IOL (COIN/MSTR/MARA/RIOT/HOOD) como cualquier otra compra IOL.
CRITERIO USD: brecha CCL baja → dolarizar ocioso; brecha alta → esperar.

Propuesta IOL #1 = mayor potencial. Si hay alertas activas de posiciones en cartera, abordarlas primero (venta urgente).` },
    ],
  });
  ...
  const saved    = [];
  const blocked  = [];   // propuestas filtradas por riesgo — se muestran pero sin botón
  let numCounter = 1;

  // ── Position sizing dinámico según cantidad de señales técnicas ─────────────
  // Más convicción = más capital. Claude sugiere un pct base; nosotros lo ajustamos.
  function dynamicPct(rawSignals, claudePct) {
    const n = rawSignals.length;
    let pct;
    if      (n >= 6) pct = 0.20;
    else if (n >= 5) pct = 0.17;
    else if (n >= 4) pct = 0.14;
    else if (n >= 3) pct = 0.11;
    else if (n >= 2) pct = 0.08;
    else             pct = 0.05;
    // Tomamos el mayor entre el dinámico y el sugerido por Claude, capped al 25%
    return Math.min(Math.max(pct, claudePct ?? 0), 0.25);
  }
  ...
  async function saveIolSignal(p, num) {
    ...
    // ── Scale-in: si ya tenemos la posición y está ganando ─────────────────
    const posExistente = portfolioMap.get(sym);
    let pctFinal = p.pct_efectivo ?? 0.10;
    let accion   = p.dir;
    if (posExistente && p.dir === 'compra') {
      ...
      if (ppc > 0 && actual > ppc && rawSignals.length >= 3) {
        // Posición ganadora con señal fuerte → scale-in al 50% del tamaño normal
        pctFinal = dynamicPct(rawSignals, p.pct_efectivo) * 0.5;
        accion   = 'scale_in';
        console.log(`[advisor] scale-in ${sym}: posición +${(((actual - ppc) / ppc) * 100).toFixed(1)}%`);
      } else if (ppc > 0 && actual <= ppc) {
        // Posición perdedora → no agregar (no promediar a la baja)
        console.log(`[advisor] ${sym} saltada — no se promedia a la baja`);
        return;
      }
    } else {
      pctFinal = dynamicPct(rawSignals, p.pct_efectivo);
    }

    // ── Filtros de riesgo de portfolio (solo para compras nuevas) ─────────────
    if (p.dir === 'compra' && accion !== 'scale_in') {
      const sectorCheck = checkSectorConcentration(sym, titulos);
      if (!sectorCheck.ok) {
        console.log(`[advisor] ${sym} filtrado — ${sectorCheck.reason}`);
      ...
      if (!currencyCheck.ok) {
        console.log(`[advisor] ${sym} filtrado — ${currencyCheck.reason}`);
        blocked.push({ num, simbolo: sym, dir: p.dir, razon: currencyCheck.reason, rawSignals });
        return;
      }
    }

    const cantidad = precio && (p.dir === 'compra' || accion === 'scale_in') && efectivo > 0
      ? Math.floor(efectivo * pctFinal / precio) : null;

    const techSignals = rawSignals.map(s => s.detail);
    ...
    try {
      const row = await savePendingSignal({
        ...
        ef_pre: efectivo,
        signals: [`propuesta:${num}`, `pct:${pctFinal.toFixed(3)}`, `mercado:${mercado}`, `accion:${accion}`, ...techSignals],
        status: 'pending',
      });
      saved.push({
        num, simbolo: sym, dir: p.dir, accion, precio, cantidad, pct: pctFinal,
        id: row?.id ?? null,
        rawSignals,
      });
    } catch (e) {
  ...
  const btnRow = saved.map(s => ({ text: `✅ Si ${s.num}`, callback_data: `si:${s.num}` }));
  const inlineButtons = [btnRow, [{ text: '❌ Cancelar todo', callback_data: 'no' }]];

  await sendMessageWithButtons(
    `🔔 *PROPUESTAS*\n\n${propLines}${blockedLines}\n\nTocá un botón o respondé */si 1*, */si 2*... para ejecutar. */no* cancela todo.`,
    inlineButtons,
  );
}
