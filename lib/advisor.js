...
import { savePendingSignal, cancelAllPending, getRecentTrades, getRecentProposals, getCustomWatchlist, addCooldown, saveProposal } from './supabase.js';
...
💡 *PROPUESTAS*
Hasta 4 propuestas de IOL (acciones BCBA, CEDEARs, bonos, CEDEARs crypto). Evaluá también los CANDIDATOS A RECOMPRA del historial — si tienen señales y no hay cooldown, son oportunidades concretas. Máx 5 posiciones totales en cartera.
...
Propuesta #1 = mayor urgencia/potencial. Alertas de stop-loss activas primero.` },
  ...
  for (const p of rawProposals) {
    ...
    try {
      ...
      // Guardar en propuestas_log para tracking de accuracy (resultado a 7d y 30d)
      await saveProposal({
        simbolo: p.sym, dir: p.dir,
        precio_prop: p.precio ?? 0,
        pct_efectivo: p.pctFinal ?? 0,
        signals: techSignals,
      }).catch(() => {});
    } catch (e) {
