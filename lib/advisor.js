...
import { savePendingSignal, cancelAllPending, getRecentTrades, saveProposal, getPerformanceStats, getRecentProposals, getCustomWatchlist, logTrade, updateSignalStatus } from './supabase.js';
...
Propuesta IOL #1 = mayor potencial. Si hay alertas activas, abordarlas primero.` },
  ...
  // ── Auto-ejecución: compras con ≥3 señales técnicas y ≤15% del efectivo ──────
  const AUTO_MIN_SIGNALS = 3;
  const AUTO_MAX_PCT     = 0.15;
  const autoExecuted     = new Set();

  const autoMsgs = [];
  for (const s of saved) {
    if (s.dir !== 'compra') continue;
    if ((s.rawSignals?.length ?? 0) < AUTO_MIN_SIGNALS) continue;
    if (s.pct > AUTO_MAX_PCT) continue;
    ...
    try {
      ...
      await logTrade({
        ...
        precio:       precioLimite,
        cantidad:     s.cantidad,
        monto:        Math.round(precioLimite * s.cantidad),
        senales:      s.rawSignals.map(sig => sig.detail ?? sig),
        efectivo_pre: efectivo,
      }).catch(() => {});
      if (s.id) await updateSignalStatus(s.id, 'auto_ejecutado').catch(() => {});
      autoExecuted.add(s.num);
      autoMsgs.push(
        `⚡ *${s.simbolo}* — AUTO-COMPRA ejecutada\n` +
        `${s.cantidad} u. @ $${precioLimite.toLocaleString('es-AR')} ` +
        `(≈$${Math.round(precioLimite * s.cantidad).toLocaleString('es-AR')} ARS)\n` +
        `Señales: ${s.rawSignals.map(sig => sig.detail ?? sig).join(' | ')}`
      );
      console.log(`[advisor] auto-compra ejecutada: ${s.simbolo}`);
    } catch (e) {
      console.log(`[advisor] auto-compra ${s.simbolo} error:`, e.message);
    }
  }

  if (autoMsgs.length > 0) {
    await sendMessage(`⚡ *AUTO-COMPRAS EJECUTADAS*\n\n${autoMsgs.join('\n\n')}`);
  }

  // Filtrar las auto-ejecutadas de las propuestas manuales
  const savedManual = saved.filter(s => !autoExecuted.has(s.num));

  // Armar mensaje agrupado por categoría
  if (savedManual.length === 0) {
    // Todas las propuestas fueron auto-ejecutadas
    return;
  }

  const CRYPTO_CEDEARS = new Set(['COIN','MSTR','MARA','RIOT','HOOD']);
  const iolSaved    = savedManual.filter(s => (s.dir === 'compra' || s.dir === 'venta') && !CRYPTO_CEDEARS.has(s.simbolo));
  const cryptoSaved = savedManual.filter(s => (s.dir === 'compra' || s.dir === 'venta') && CRYPTO_CEDEARS.has(s.simbolo));
  const usdSaved    = savedManual.filter(s => s.dir === 'dolar');

  function lineFor(s) {
    const emojis = ['1️⃣','2️⃣','3️⃣','4️⃣','5️⃣'];
    const emoji = s.num <= 5 ? emojis[s.num - 1] : `${s.num}.`;
    if (s.dir === 'dolar') return `${emoji} *${s.simbolo}* — 💵 DOLARIZAR ${(s.pct * 100).toFixed(0)}%`;
  ...
  if (cryptoSaved.length) sections.push(`🪙 *CRYPTO (via IOL):*\n${cryptoSaved.map(lineFor).join('\n')}`);
  if (usdSaved.length)    sections.push(`💵 *USD:*\n${usdSaved.map(lineFor).join('\n')}`);

  // Armar botones inline: una fila con los "Si N" y otra con Cancelar
  const siButtons = savedManual.map(s => ({ text: `✅ Si ${s.num}`, callback_data: `si:${s.num}` }));
  const rows = [];
  // Máximo 3 botones por fila
  for (let i = 0; i < siButtons.length; i += 3) rows.push(siButtons.slice(i, i + 3));
  rows.push([{ text: '❌ Cancelar todo', callback_data: 'no' }]);
