...
Si ninguno aplica, las 3 propuestas pueden ser todas compras. Para ventas, pct_efectivo = 0.`,
  ...
  for (let i = 0; i < propuestas.length && i < 3; i++) {
    ...
    console.log(`[advisor] ${sym}: precio=${precio} cantidad=${cantidad} efectivo=${efectivo}`);

    if (p.dir === 'compra' && (!cantidad || cantidad < 1)) {
      console.log(`[advisor] ${sym}: cantidad insuficiente, skip`);
      continue;
    }
    ...
    try {
      ...
      console.log(`[advisor] ${sym}: guardado en pending_signals`);
    } catch (e) {
