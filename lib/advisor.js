...
import { isBinanceConfigured, getBinanceBalance } from './binance.js';

export async function runAdvisor() {
  const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_KEY });

  const binanceConf = isBinanceConfigured();
  const [token, research, recentTrades, dolar, recentProposals, lastTrade, cryptoPrices, cryptoTrending, binanceUsdt] = await Promise.all([
    getToken(),
    fetchMarketResearch(),
    getRecentTrades(10),
    getDolarData(),
    ...
    getCryptoPrices(),
    getCryptoTrending(),
    binanceConf ? getBinanceBalance('USDT') : Promise.resolve(null),
  ]);
...
CRYPTO (ejecutado automáticamente via Binance${binanceConf ? ` — USDT disponible: $${(binanceUsdt ?? 0).toFixed(2)}` : ' — sin Binance configurado, ejecución manual'}):
${cryptoContext}
${cryptoHoldingsText}
...
Crypto operable (Binance): BTC, ETH, SOL, BNB, XRP, MATIC, ADA, DOGE
  ...
  const proponerTool = {
    ...
    input_schema: {
      ...
      properties: {
        propuestas_iol: {
          type: 'array',
          description: 'Hasta 3 propuestas de compra/venta en IOL. Array vacío [] si no hay oportunidad.',
          maxItems: 3,
          items: {
            type: 'object',
            properties: {
              simbolo: { type: 'string', description: 'Ticker exacto del WATCHLIST' },
        ...
        propuesta_crypto: {
          description: 'Una propuesta crypto via Binance. null si no hay señal clara.',
          oneOf: [
            {
              type: 'object',
              properties: {
                simbolo: { type: 'string', description: 'BTC, ETH, SOL, BNB, XRP, MATIC, ADA, DOGE' },
                dir: { type: 'string', enum: ['crypto', 'crypto_venta'], description: '"crypto_venta" solo si aparece en holdings estimados' },
                pct_usdt: { type: 'number', description: 'Fracción del USDT disponible en Binance (0.10-0.50). Ventas: 0.' },
              },
              required: ['simbolo', 'dir', 'pct_usdt'],
            },
            { type: 'null' },
          ],
        },
        propuesta_usd: {
          description: 'Una propuesta de dolarización. null si no aplica.',
          oneOf: [
            {
              type: 'object',
              properties: {
...
Propuesta IOL #1 = mayor potencial. Si hay alertas activas, abordarlas primero.` },
  ...
  const propuesta_crypto = input.propuesta_crypto ?? null;
  ...
  async function saveIolSignal(p, num) {
    ...
    try {
      ...
      saved.push({ num, simbolo: sym, dir: p.dir, precio, cantidad, pct: p.pct_efectivo ?? 0 });
    } catch (e) {
  ...
  // IOL proposals (1-3)
  for (const p of propuestas_iol.slice(0, 3)) {
    await saveIolSignal(p, numCounter++);
  }

  // Crypto proposal (next num)
  if (propuesta_crypto) {
    const sym = propuesta_crypto.simbolo?.toUpperCase();
    const pct = propuesta_crypto.pct_usdt ?? 0;
    try {
      await savePendingSignal({
        simbolo: sym, dir: propuesta_crypto.dir,
        precio: 0, cantidad: null,
        ef_pre: efectivo,
        signals: [`propuesta:${numCounter}`, `pct_usdt:${pct}`, `mercado:binance`],
        status: 'pending',
      });
      saved.push({ num: numCounter, simbolo: sym, dir: propuesta_crypto.dir, precio: null, cantidad: null, pct });
    } catch (e) { console.log('[advisor] crypto save error:', e.message); }
    numCounter++;
  }
  ...
  // Armar mensaje agrupado por categoría
  const iolSaved    = saved.filter(s => s.dir === 'compra' || s.dir === 'venta');
  const cryptoSaved = saved.filter(s => s.dir === 'crypto' || s.dir === 'crypto_venta');
  const usdSaved    = saved.filter(s => s.dir === 'dolar');

  function lineFor(s) {
    const emoji = s.num <= 5 ? ['1️⃣','2️⃣','3️⃣','4️⃣','5️⃣'][s.num - 1] : `${s.num}.`;
    if (s.dir === 'dolar') {
      return `${emoji} *${s.simbolo}* — 💵 DOLARIZAR ${(s.pct * 100).toFixed(0)}%`;
    }
    if (s.dir === 'crypto' || s.dir === 'crypto_venta') {
      const accion = s.dir === 'crypto_venta' ? '📉 VENTA' : '🪙 COMPRA';
      const pctStr = s.pct > 0 ? ` ${(s.pct * 100).toFixed(0)}% del USDT` : ' posición completa';
      return `${emoji} *${s.simbolo}* — ${accion}:${pctStr}`;
    }
    if (s.dir === 'venta') return `${emoji} *${s.simbolo}* — 📉 VENTA: posición completa`;
    const montoStr = (s.cantidad >= 1 && s.precio)
      ? `${s.cantidad} u. @ $${s.precio} (≈$${Math.round(s.cantidad * s.precio).toLocaleString('es-AR')} ARS)`
      : `≈$${Math.round(efectivo * s.pct).toLocaleString('es-AR')} ARS (${(s.pct * 100).toFixed(0)}%)`;
    return `${emoji} *${s.simbolo}* — 📈 COMPRA: ${montoStr}`;
  }

  const sections = [];
  if (iolSaved.length)    sections.push(`📈 *IOL:*\n${iolSaved.map(lineFor).join('\n')}`);
  if (cryptoSaved.length) sections.push(`🪙 *CRYPTO (Binance):*\n${cryptoSaved.map(lineFor).join('\n')}`);
  if (usdSaved.length)    sections.push(`💵 *USD:*\n${usdSaved.map(lineFor).join('\n')}`);

  const nums = saved.map(s => s.num).join(', ');
  await sendMessage(
    `🚨 *PROPUESTAS LISTAS*\n\n${sections.join('\n\n')}\n\n` +
    `Respondé */si ${saved[0].num}* para una, */si 1 3* para varias.\n` +
