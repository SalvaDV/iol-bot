...
import { savePendingSignal } from './supabase.js';

function allocationPct(score) {
  if (score >= 4) return 0.30;
...
Sé directo y específico. Si no hay oportunidades claras, decilo. Formato Markdown para Telegram. Máximo 550 palabras.`,
  ...
  // Elegir la mejor señal técnica y guardarla como pending para que el usuario pueda confirmar
  ...
  const cantidad = best.dir === 'compra' ? Math.floor(efectivo * pct / precio) : null;

  if (best.dir === 'compra' && (!cantidad || cantidad < 1)) return;
  ...
  const montoStr = best.dir === 'compra'
    ? `${cantidad} u. — $${(cantidad * precio).toLocaleString('es-AR')} ARS (${(pct * 100).toFixed(0)}% del efectivo)`
    : `posición completa`;
  ...
  await sendMessage(
    ...
    `💵 Precio: $${precio}\n` +
    `📦 Cantidad: ${montoStr}\n` +
    `📊 Señales confirmadas: ${best.score}/4\n\n` +
