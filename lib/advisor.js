...
ESTRUCTURA:
• propuestas_iol: hasta ${Math.max(0, 4 - titulos.length + titulos.filter(t => propuestas_iol?.some?.(p => p.dir === 'venta' && p.simbolo === t.simbolo)).length)} compras/ventas. [] si no hay señales claras.
