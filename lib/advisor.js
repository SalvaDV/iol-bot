...
Máximo 400 palabras. Sé directo y específico.`,
  ...
  for (let i = 0; i < propuestas.length && i < 3; i++) {
    ...
    if (!precio) {
      try {
        ...
        precio = cot.ultimoPrecio || cot.ultimo || cot.precio || null;
        console.log(`[advisor] ${sym} cotizacion fallback:`, JSON.stringify(cot).slice(0, 200));
      } catch (e) {
    ...
    if (!precio) { console.log(`[advisor] ${sym}: sin precio, skip`); continue; }

    // Guardar pct_efectivo para recalcular cantidad al ejecutar (si efectivo cambia)
    const cantidad = p.dir === 'compra' && p.pct_efectivo && efectivo > 0
      ? Math.floor(efectivo * p.pct_efectivo / precio)
      : null;

    console.log(`[advisor] ${sym}: precio=${precio} cantidad=${cantidad} efectivo=${efectivo} pct=${p.pct_efectivo}`);
    ...
    try {
      ...
      console.log(`[advisor] ${sym}: guardado en pending_signals OK`);
    } catch (e) {
  ...
  const lines = saved.map(s => {
    ...
    if (s.dir === 'venta') {
      ...
      montoStr = `${s.cantidad} u. @ $${s.precio} = $${(s.cantidad * s.precio).toLocaleString('es-AR')} ARS`;
    } else {
      montoStr = `${(s.pct * 100).toFixed(0)}% del efectivo @ $${s.precio}`;
    }
