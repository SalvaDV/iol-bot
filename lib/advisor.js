...
import { savePendingSignal, cancelAllPending, getRecentTrades, saveProposal, getPerformanceStats, getRecentProposals } from './supabase.js';
...
export async function runAdvisor() {
  ...
  const [token, research, recentTrades, dolar, recentProposals, cryptoPrices, cryptoTrending] = await Promise.all([
    getToken(),
    fetchMarketResearch(),
    getRecentTrades(10),
    getDolarData(),
  ...
  // Portfolio y cuenta primero para poder pasar los símbolos del portafolio a fetchAllTechnicals
  const [portfolio, cuenta] = await Promise.all([
    getPortfolio(token),
    getCuenta(token),
  ]);
  ...
  // Incluir símbolos del portafolio en el análisis técnico (no solo el WATCHLIST fijo)
  const portfolioSyms = (portfolio.titulos || []).map(t => t.simbolo?.toUpperCase()).filter(Boolean);
  const technicals = await fetchAllTechnicals(token, portfolioSyms);
      ...
      properties: {
        propuestas_iol: {
          ...
          items: {
            type: 'object',
            properties: {
              simbolo: { type: 'string', description: 'Ticker exacto del WATCHLIST. Para crypto usá COIN, MSTR, MARA, RIOT o HOOD.' },
...
Propuesta IOL #1 = mayor potencial. Si hay alertas activas, abordarlas primero.` },
  ...
  const iolSaved    = saved.filter(s => (s.dir === 'compra' || s.dir === 'venta') && !CRYPTO_CEDEARS.has(s.simbolo));
  const cryptoSaved = saved.filter(s => (s.dir === 'compra' || s.dir === 'venta') && CRYPTO_CEDEARS.has(s.simbolo));
  ...
  const siButtons = saved.map(s => ({ text: `✅ Si ${s.num}`, callback_data: `si:${s.num}` }));
  const rows = [];
