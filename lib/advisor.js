...
Si no se cumple ninguno con certeza, NO propongas venta — poné compra en su lugar. Las 3 propuestas pueden ser todas compras.`,
  ...
  // Separar narrativa del JSON — regex permisivo (acepta \r\n, espacios, etc.)
  ...
  if (jsonMatch) {
    try {
      ...
      console.log('[advisor] propuestas del JSON:', JSON.stringify(propuestas));
    } catch (e) {
      console.log('[advisor] JSON parse error:', e.message, '| raw:', jsonMatch[1].slice(0, 200));
    }
  } else {
    console.log('[advisor] jsonMatch null — Claude no devolvió JSON. Usando fallback técnico.');
  }

  // Fallback: si Claude no devolvió JSON válido, usar la mejor señal técnica
  if (propuestas.length === 0) {
    const withSignals = technicals.filter(t => t?.dir);
    if (withSignals.length === 0) {
      console.log('[advisor] Sin señales técnicas tampoco. Nada que guardar.');
      return;
    }
    const scored = withSignals.map(t => ({
      ...t,
      score: t.signals.filter(s => s.dir === t.dir).length,
    })).sort((a, b) => b.score - a.score);

    // Hasta 3 mejores señales como propuestas fallback
    propuestas = scored.slice(0, 3).map(t => ({
      simbolo: t.sym,
      dir: t.dir,
      pct_efectivo: t.dir === 'compra' ? (t.score >= 3 ? 0.20 : 0.12) : null,
    }));
    console.log('[advisor] Fallback propuestas:', JSON.stringify(propuestas));
  }
