...
export async function runAdvisor() {
  ...
  const [token, research, recentTrades, dolar, recentProposals] = await Promise.all([
    getToken(),
    fetchMarketResearch(),
    getRecentTrades(10),
    getDolarData(),
  ...
  const efectivo =
    ...
    cuenta.saldo ??
    0;
  console.log('[advisor] cuenta fields:', JSON.stringify({ c0_moneda: c0?.moneda, c0_disp: c0?.disponible, c1_moneda: c1?.moneda, c1_disp: c1?.disponible, efectivo }));
  ...
  const contexto = `Sos un asesor financiero para el mercado argentino (BCBA y CEDEARs). El usuario tiene conocimiento intermedio — entiende conceptos básicos pero está aprendiendo. Explicá el razonamiento detrás de cada recomendación para que pueda entender y aprender. Respondés siempre en español. Hoy es ${now}.${diversificacionNote}
...
DIVERSIFICACIÓN OBLIGATORIA: Las 3 propuestas deben ser de símbolos distintos entre sí. Si hay instrumentos repetidos en sesiones recientes (indicados arriba), preferí alternativas con señales similares — por ejemplo, si GGAL fue propuesto 3 veces, elegí otro banco (BMA, SUPV) o un sector diferente. Mezclá tipos: acciones BCBA + CEDEARs + bonos cuando las señales lo permitan.
