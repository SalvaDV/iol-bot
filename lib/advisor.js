...
import { canBuy, canSell } from './riskManager.js';
...
Propuesta IOL #1 = mayor potencial. Si hay alertas activas, abordarlas primero.` },
  ...
  const ejecutadas    = [];
  const bloqueadas    = [];
  let   buyCount      = 0;   // solo compras ejecutadas (para el contador de posiciones abiertas)
  ...
  for (const s of saved) {
    ...
    if (s.dir === 'venta') {
      ...
      if (!sellCheck.allowed) {
        bloqueadas.push(`📉 *${s.simbolo}* venta bloqueada — ${sellCheck.reason}`);
        continue;
      }
    ...
    if (s.dir === 'compra') {
      if (!s.precio || !s.cantidad || s.cantidad < 1) continue;
      const check = await canBuy({ simbolo: s.simbolo, pct: s.pct, efectivo, openPositions: openPositions + buyCount });
      if (!check.allowed) {
      ...
      try {
        ...
        if (s.id) await updateSignalStatus(s.id, 'auto_ejecutado').catch(() => {});
        const etiqueta = s.accion === 'scale_in' ? '📈➕ SCALE-IN' : `📈 COMPRA (${s.rawSignals.length} señales)`;
        const warningLine = check.warning ? `\n⚠️ ${check.warning}` : '';
        ejecutadas.push(
          `${etiqueta} *${s.simbolo}* — ${s.cantidad} u. @ $${precioLimite.toLocaleString('es-AR')} ` +
          `(≈$${Math.round(precioLimite * s.cantidad).toLocaleString('es-AR')} ARS | ${(s.pct * 100).toFixed(0)}% efectivo)\n` +
          `↳ ${s.rawSignals.map(sig => sig.detail ?? sig).join(' | ')}${warningLine}`
        );
        buyCount++;
