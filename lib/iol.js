...
// Caché en memoria del token IOL (válido mientras la instancia Vercel esté caliente)
...
export async function getToken() {
  ...
  _tokenExpiry = now + 25 * 60 * 1000; // 25 min (IOL emite tokens de 30 min)
  return _tokenCache;
}
...
/**
 * Extrae el efectivo disponible para operar en ARS desde el response de /estadocuenta.
 * IOL separa la liquidez por plazo; el bot opera con t2 así que usamos hrs24.disponibleOperar
 * que incluye fondos inmediatos + liquidaciones a 24hs (el total real operable).
 */
export function extractEfectivo(cuenta) {
  const cuentaPesos = cuenta.cuentas?.find(c => c.moneda?.toLowerCase().includes('peso'));
  const saldo24 = cuentaPesos?.saldos?.find(s => s.liquidacion === 'hrs24');
  return (
    saldo24?.disponibleOperar ??
...
/**
 * Normaliza la respuesta de getPortfolio() a un array uniforme.
 * IOL devuelve `activos[]` con el símbolo dentro de `titulo.simbolo`;
 * versiones anteriores devolvían `titulos[]` con `simbolo` directo.
 */
export function normalizePortfolio(portfolio) {
  const items = portfolio.activos || portfolio.titulos || [];
  return items.map(item => ({
    simbolo:      (item.titulo?.simbolo  ?? item.simbolo  ?? '').toUpperCase(),
    descripcion:  item.titulo?.descripcion ?? item.descripcion ?? '',
    cantidad:     item.cantidad    ?? 0,
    ultimoPrecio: item.ultimoPrecio ?? item.precioActual ?? null,
    variacionDiaria: item.variacionDiaria ?? null,
    ppc:          item.ppc ?? item.precioPromedio ?? item.costoPromedio ?? null,
    valorizado:   item.valorizado  ?? null,
  }));
}

/** Busca un instrumento por ticker y devuelve nombre, precio y mercado.
 *  Lanza error si el símbolo no existe en IOL. */
export async function searchInstrumento(token, simbolo) {
  const sym = simbolo.toUpperCase().trim();
  const cot = await getCotizacion(token, sym);
  const precio = extractPrecio(cot);
  const nombre =
    cot.descripcion ?? cot.nombre ?? cot.nombreEspecie ??
    cot.nombreCompleto ?? cot.denominacion ?? sym;
  const mercado = cot._mercado || 'bcba';
  return { simbolo: sym, nombre, precio, mercado };
}
...
export async function crearOrden(token, { mercado = 'bcba', simbolo, cantidad, precio, operacion, plazo = 't2' }) {
  ...
  // Loguear siempre la respuesta raw para diagnóstico
  console.log('[crearOrden] raw response:', JSON.stringify(data).slice(0, 400));
  if (
    ...
    throw new Error(`IOL rechazó la orden: ${JSON.stringify(data).slice(0, 400)}`);
  }
  return data;
}
