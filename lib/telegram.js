...
// Mensaje con force_reply: Telegram le muestra al usuario el campo de respuesta
// apuntando a este mensaje. Así el usuario solo escribe y envía — sin comandos.
export async function sendForceReply(text) {
  const body = {
    chat_id: process.env.TG_CHAT_ID,
    text,
  ...
  const res = await tgPost('sendMessage', body);
  if (!res.ok) throw new Error(`sendForceReply failed: ${res.status}`);
  return res.json();
}
