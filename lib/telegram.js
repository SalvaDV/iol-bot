...
// Teclado fijo que aparece en la parte inferior del chat
...
// Envía un mensaje mostrando el teclado principal
export async function sendMessageWithMainKeyboard(text, parseMode = 'Markdown') {
  if (!text?.trim()) return;
  const body = {
    chat_id: process.env.TG_CHAT_ID,
    text,
  ...
  const res = await tgPost('sendMessage', body);
  if (!res.ok && parseMode) {
    const retry = await tgPost('sendMessage', { ...body, parse_mode: undefined });
    if (!retry.ok) throw new Error(`sendMessageWithMainKeyboard failed: ${retry.status}`);
    return retry.json();
  }
  if (!res.ok) throw new Error(`sendMessageWithMainKeyboard failed: ${res.status}`);
  return res.json();
}
