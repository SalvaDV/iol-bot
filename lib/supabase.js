...
// ─── Cooldowns (evita re-entrada tras stop-loss) ─────────────────────────────
export async function getCooldowns() {
  const res = await sbFetch('/cooldowns');
  if (!res.ok) return {};
  const rows = await res.json();
  const now = new Date();
  return Object.fromEntries(
    rows
      .filter(r => new Date(r.blocked_until) > now)
      .map(r => [r.simbolo, r.blocked_until])
  );
}

export async function addCooldown(simbolo, hours = 24, reason = 'stoploss') {
  const blocked_until = new Date(Date.now() + hours * 3_600_000).toISOString();
  await sbFetch(`/cooldowns?simbolo=eq.${encodeURIComponent(simbolo)}`, { method: 'DELETE' });
  await sbFetch('/cooldowns', {
    method: 'POST',
    body: JSON.stringify({ simbolo, blocked_until, reason }),
  });
}

export async function removeCooldown(simbolo) {
  await sbFetch(`/cooldowns?simbolo=eq.${encodeURIComponent(simbolo)}`, { method: 'DELETE' });
}

// ─── Configuración global del bot ────────────────────────────────────────────
export async function getBotConfig(key) {
  const res = await sbFetch(`/bot_config?key=eq.${encodeURIComponent(key)}&limit=1`);
  if (!res.ok) return null;
  const rows = await res.json();
...
export async function setBotConfig(key, value) {
  await sbFetch(`/bot_config?key=eq.${encodeURIComponent(key)}`, { method: 'DELETE' });
  await sbFetch('/bot_config', {
    method: 'POST',
...
export async function setUserState(userId, action, data = {}) {
  // DELETE + INSERT es más confiable que upsert via Prefer header en PostgREST
  ...
  if (!res.ok) {
    ...
    throw new Error(`setUserState failed: ${res.status} — ${body.slice(0, 200)}`);
  }
}
...
// ─── Trailing stop: máximos históricos por posición ──────────────────────────
// Devuelve mapa { simbolo → { high_price, partial_taken, partial_qty, partial_price } }
export async function getPositionHighs() {
  const res = await sbFetch('/position_highs');
  if (!res.ok) return {};
  const rows = await res.json();
  return Object.fromEntries(rows.map(r => [r.simbolo, {
    high_price:      r.high_price,
    partial_1_taken: r.partial_1_taken ?? false,
    partial_1_qty:   r.partial_1_qty   ?? null,
    partial_1_price: r.partial_1_price ?? null,
    partial_2_taken: r.partial_2_taken ?? false,
    partial_2_qty:   r.partial_2_qty   ?? null,
    partial_2_price: r.partial_2_price ?? null,
  }]));
}

export async function upsertPositionHigh(simbolo, fields) {
  // fields: { high_price, partial_taken?, partial_qty?, partial_price? }
  await sbFetch(`/position_highs?simbolo=eq.${encodeURIComponent(simbolo)}`, { method: 'DELETE' });
  const res = await sbFetch('/position_highs', {
    method: 'POST',
    body: JSON.stringify({ simbolo, ...fields, updated_at: new Date().toISOString() }),
  });
  if (!res.ok) throw new Error(`upsertPositionHigh failed: ${res.status}`);
}

export async function deletePositionHigh(simbolo) {
  await sbFetch(`/position_highs?simbolo=eq.${encodeURIComponent(simbolo)}`, { method: 'DELETE' });
}
