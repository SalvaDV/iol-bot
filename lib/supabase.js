...
export async function getPositionHighs() {
  ...
  if (!res.ok) return {};
  const rows = await res.json();
  return Object.fromEntries(rows.map(r => [r.simbolo, r.high_price]));
}

export async function upsertPositionHigh(simbolo, high_price) {
  // DELETE + INSERT para evitar problemas de upsert en PostgREST
  await sbFetch(`/position_highs?simbolo=eq.${encodeURIComponent(simbolo)}`, { method: 'DELETE' });
  const res = await sbFetch('/position_highs', {
    method: 'POST',
    body: JSON.stringify({ simbolo, high_price, updated_at: new Date().toISOString() }),
  });
